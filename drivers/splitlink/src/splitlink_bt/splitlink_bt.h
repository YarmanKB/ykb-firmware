...
static inline void splitlink_bt_connect_work_handler(struct k_work *work) {
    struct splitlink_bt_data *data =
        CONTAINER_OF(work, struct splitlink_bt_data, connect_work);

    STRUCT_SECTION_FOREACH(splitlink_cb, callback) {
        if (callback->connect_cb) {
            callback->connect_cb(data->dev);
        }
    }
}
...
static inline void splitlink_bt_disconnect_work_handler(struct k_work *work) {
    struct splitlink_bt_data *data =
        CONTAINER_OF(work, struct splitlink_bt_data, disconnect_work);

    STRUCT_SECTION_FOREACH(splitlink_cb, callback) {
        if (callback->disconnect_cb) {
            callback->disconnect_cb(data->dev);
        }
    }
}
    ...
    while (k_msgq_get(data->rx_msgq, &packet, K_NO_WAIT) == 0) {
        STRUCT_SECTION_FOREACH(splitlink_cb, callback) {
            if (callback->on_receive_cb) {
                callback->on_receive_cb(data->dev, packet.data, packet.len);
            }
        }
    }
}
