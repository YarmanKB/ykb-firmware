#define DT_DRV_COMPAT splitlink_bt_central
...
static int splitlink_bt_send(const struct device *dev, uint8_t *data,
    ...
    if (!dev_data->connected || !dev_data->conn || dev_data->rx_handle == 0) {
        return -ENOTCONN;
    }

    return bt_gatt_write_without_response(dev_data->conn, dev_data->rx_handle,
                                          data, data_len, false);
}
...
static void splitlink_bt_device_found(const bt_addr_le_t *addr, int8_t rssi,
    ...
    if (((struct splitlink_bt_data *)splitlink_bt_dev->data)->conn) {
        return;
    }
    ...
    err = bt_conn_le_create(
        ...
        &((struct splitlink_bt_data *)splitlink_bt_dev->data)->conn);
    if (err) {
        LOG_ERR("Splitlink BT connect failed (%d), restarting scan", err);
        ((struct splitlink_bt_data *)splitlink_bt_dev->data)->conn = NULL;
...
int splitlink_bt_start(void) {
    ...
    dev_data = splitlink_bt_dev->data;
    dev_data->ready = true;
...
static void splitlink_bt_connected(struct bt_conn *conn, uint8_t err) {
    ...
    if (conn_param_err && conn_param_err != -EALREADY) {
        LOG_WRN("Splitlink BT conn param update failed (%d)", conn_param_err);
    } else {
    ...
    if (phy_err && phy_err != -EALREADY) {
        LOG_WRN("Splitlink BT PHY update failed (%d)", phy_err);
    } else {
    ...
    if (data_len_err && data_len_err != -EALREADY) {
        LOG_WRN("Splitlink BT data len update failed (%d)", data_len_err);
    } else {
...
static DEVICE_API(splitlink, splitlink_bt_central_api) = {
    .send = splitlink_bt_send,
};

#define SPLITLINK_BT_CENTRAL_DEFINE(inst)                                      \
    ...
    DEVICE_DT_INST_DEFINE(                                                     \
        ...
        CONFIG_SPLITLINK_BT_INIT_PRIORITY, &splitlink_bt_central_api);

DT_INST_FOREACH_STATUS_OKAY(SPLITLINK_BT_CENTRAL_DEFINE)
