...
static void splitlink_bt_request_link_updates(struct bt_conn *conn) {
    ...
    if (err && err != -EALREADY) {
        LOG_WRN("Splitlink BT peripheral conn param update failed (%d)", err);
    } else {
    ...
    if (err && err != -EALREADY) {
        LOG_WRN("Splitlink BT peripheral PHY 2M update failed (%d)", err);
    } else {
    ...
    if (err && err != -EALREADY) {
        LOG_WRN("Splitlink BT peripheral data len update failed (%d)", err);
    } else {
...
static int splitlink_bt_send(uint8_t *data, size_t data_len) {
    ...
    if (!splitlink_bt_data.connected || !splitlink_bt_data.conn ||
        !splitlink_bt_data.notify_enabled) {
        return -ENOTCONN;
    }

    return bt_gatt_notify(splitlink_bt_data.conn, &splitlink_bt_svc.attrs[2],
                          data, data_len);
}
